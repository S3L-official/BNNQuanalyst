...
QE_SDComputerSylvan::QE_SDComputerSylvan(Query *q, Options *opt, BNNetwork *bnn, string relaPath) : _bnn() {
    _query = q;
    _opt = opt;
    _bnn = bnn;
    _SD = new vector<int>;
...
QE_SDComputerSylvan::QE_SDComputerSylvan() : _bnn() {

}


int QE_SDComputerSylvan::forwardPredict(vector<int> input) {
...
void QE_SDComputerSylvan::solve() {
...
//    _BddVec = encoder.encode2targetBDD(_query->_target_class);
    ...
    for (int num = 0; num < _BddVec->size(); ++num) { /* compute SD for each target class */
        double target_result = (*_BddVec)[num].SatCount(input_size);
        ...
        if (rightRatio < 1 - tole) /* less then decrease r */{
            ...
            while (SD >= 0) {
                ...
                sylvan::Bdd new_IR = encoder.Cudd_bddCCEncoding(&_query->_instance, input_size - SD, 0, false);
                sylvan::Bdd prod = new_IR * (*_BddVec)[num];
                target_result = prod.SatCount(input_size);
            ...
            while (SD <= input_size) {
                ...
                sylvan::Bdd new_IR_super = encoder.Cudd_bddCCEncoding(&_query->_instance, input_size - SD, 0, false);
                sylvan::Bdd new_IR_sub = encoder.Cudd_bddCCEncoding(&_query->_instance, SD, 0, true);
                sylvan::Bdd div = new_IR_super * new_IR_sub;
                encoder.setInputRegion(div);
                vector<sylvan::Bdd> *new_vec_bdd = encoder.encode2targetBDD(predict_vec);
                double sup_target_result = (*new_vec_bdd)[0].SatCount(input_size);
 ...
 */
void QE_SDComputerSylvan::logPrint(char *filename) {
    int input_size = _bnn->get_input_size();
    FILE *outfile;
